const comments = [

{
    "_id": "5b21ca3eeb7f6fbccd471815",
    "professor": "John Alexis Guerra Gómez",
    "courseId": "CS 5610",
    "courseName": "Web Development",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471816",
    "professor": "Ricardo Baeza-Yates",
    "courseId": "CS 5800",
    "courseName": "Algorithms",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471817",
    "professor": "Ricardo Baeza-Yates",
    "courseId": "CS 5600",
    "courseName": "Computer Systems",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471818",
    "professor": "Abraham Bagherjeiran",
    "courseId": "CS 7610",
    "courseName": "Foundations of Distributed Systems",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471819",
    "professor": "Abraham Bagherjeiran",
    "courseId": "CS 6200",
    "courseName": "Information Retrieval",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471820",
    "professor": "Suhabe Bugrara",
    "courseId": "CS 5520", 
    "courseName": "Mobile Application Development",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471821",
    "professor": "Suhabe Bugrara",
    "courseId": "CS 5500", 
    "courseName": "Managing Software Development",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471822",
    "professor": "Gao Hechen",
    "courseId": "CS 5400",
    "courseName": "Principles of Programming Language",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471823",
    "professor": "Gao Hechen",
    "courseId": "CS 5200",
    "courseName": "Database Management Systems",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471824",
    "professor": "Gao Hechen",
    "courseId": "CS 5150",
    "courseName": "Game Artificial Intelligence",
    "username": "xz2969",
    "comment": "Good"

},
{
    "_id": "5b21ca3eeb7f6fbccd471825",
    "professor": "Matthew Goodwin",
    "courseId": "CS 5100",
    "courseName": "Foundations of Artificial Intelligence",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471826",
    "professor": "Matthew Goodwin",
    "courseId": "CS 5082", 
    "courseName": "Privacy and Security of User Accounts",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471827",
    "professor": "Matthew Goodwin",
    "courseId": "CS 5010",
    "courseName": "Programming Design Paradigm",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471828",
    "professor": "Matthew Goodwin",
    "courseId": "CS 5011", 
    "courseName": "Recitation for CS 5010",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471829",
    "professor": "Gust Philip",
    "courseId": "CS 5001",
    "courseName": "Intensive Foundations of Computer Science",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471830",
    "professor": "Gust Philip",
    "courseId": "CS 5002",
    "courseName": "Discrete and Data Structures",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471831",
    "professor": "Gust Philip",
    "courseId": "CS 5003", 
    "courseName": "Recitation for CS 5001",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471832",
    "professor": "Gust Philip",
    "courseId": "CS 5004",
    "courseName": "Object-Oriented Design",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471833",
    "professor": "Gust Philip",
    "courseId": "CS 5005",
    "courseName": "Recitation for CS 5004",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471834",
    "professor": "Gust Philip",
    "courseId": "CS 5006",
    "courseName": "Algorithms",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471835",
    "professor": "Gust Philip",
    "courseId": "CS 5007",
    "courseName": "Computer Systems",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471836",
    "professor": "Gust Philip",
    "courseId": "CS 5600",
    "courseName": "Computer Systems",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471837",
    "professor": "Gust Philip",
    "courseId": "CS 5800",
    "courseName": "Algorithms",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471838",
    "professor": "Ed Katz",
    "courseId": "CS 5001",
    "courseName": "Intensive Foundations of Computer Science",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471839",
    "professor": "Ed Katz",
    "courseId": "CS 5002", 
    "courseName": "Discrete and Data Structures",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471840",
    "professor": "Ed Katz",
    "courseId": "CS 5003", 
    "courseName": "Recitation for CS 5001",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471841",
    "professor": "Ed Katz",
    "courseId": "CS 5004",
    "courseName": "Object-Oriented Design",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471842",
    "professor": "Ed Katz",
    "courseId": "CS 5005",
    "courseName": "Recitation for CS 5004",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471843",
    "professor": "Ed Katz",
    "courseId": "CS 5006", 
    "courseName": "Algorithms",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471844",
    "professor": "Ed Katz",
    "courseId": "CS 5007",
    "courseName": "Computer Systems",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471845",
    "professor": "Ed Katz",
    "courseId": "CS 5600",
    "courseName": "Computer Systems",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471846",
    "professor": "Ed Katz",
    "courseId": "CS 5800",
    "courseName": "Algorithms",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471847",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 6200",
    "courseName": "Information Retrieval",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471848",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 7610",
    "courseName": "Foundations of Distributed Systems",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471849",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 5150",
    "courseName": "Game Artificial Intelligence",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471850",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 5200",
    "courseName": "Database Management Systems",
    "username": "xz2969",
    "comment": "Good"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471851",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 5400", 
    "courseName": "Principles of Programming Language",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471852",
    "professor": "Prasad Saripalli",
    "courseId": "CS 5500",
    "courseName": "Managing Software Development",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471853",
    "professor": "Prasad Saripalli",
    "courseId": "CS 5520",
    "courseName": "Mobile Application Development",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471854",
    "professor": "Chuhua Zhong",
    "courseId": "CS 5610", 
    "courseName": "Web Development",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471855",
    "professor": "Chuhua Zhong",
    "courseId": "CS 6200",
    "courseName": "Information Retrieval", 
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471856",
    "professor": "Chuhua Zhong",
    "courseId": "CS 7610", 
    "courseName": "Foundations of Distributed Systems",
    "username": "xz2969",
    "comment": "Good"
},
{
    "_id": "5b21ca3eeb7f6fbccd471857",
    "professor": "John Alexis Guerra Gómez",
    "courseId": "CS 5610",
    "courseName": "Web Development",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471858",
    "professor": "Ricardo Baeza-Yates",
    "courseId": "CS 5800",
    "courseName": "Algorithms",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471859",
    "professor": "Ricardo Baeza-Yates",
    "courseId": "CS 5600",
    "courseName": "Computer Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471860",
    "professor": "Abraham Bagherjeiran",
    "courseId": "CS 7610",
    "courseName": "Foundations of Distributed Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471861",
    "professor": "Abraham Bagherjeiran",
    "courseId": "CS 6200",
    "courseName": "Information Retrieval",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471862",
    "professor": "Suhabe Bugrara",
    "courseId": "CS 5520", 
    "courseName": "Mobile Application Development",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471863",
    "professor": "Suhabe Bugrara",
    "courseId": "CS 5500", 
    "courseName": "Managing Software Development",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471864",
    "professor": "Gao Hechen",
    "courseId": "CS 5400",
    "courseName": "Principles of Programming Language",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471865",
    "professor": "Gao Hechen",
    "courseId": "CS 5200",
    "courseName": "Database Management Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471866",
    "professor": "Gao Hechen",
    "courseId": "CS 5150",
    "courseName": "Game Artificial Intelligence",
    "username": "mengbanana",
    "comment": "Too hard"

},
{
    "_id": "5b21ca3eeb7f6fbccd471867",
    "professor": "Matthew Too hardwin",
    "courseId": "CS 5100",
    "courseName": "Foundations of Artificial Intelligence",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471868",
    "professor": "Matthew Too hardwin",
    "courseId": "CS 5082", 
    "courseName": "Privacy and Security of User Accounts",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471869",
    "professor": "Matthew Too hardwin",
    "courseId": "CS 5010",
    "courseName": "Programming Design Paradigm",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471870",
    "professor": "Matthew Too hardwin",
    "courseId": "CS 5011", 
    "courseName": "Recitation for CS 5010",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471871",
    "professor": "Gust Philip",
    "courseId": "CS 5001",
    "courseName": "Intensive Foundations of Computer Science",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471872",
    "professor": "Gust Philip",
    "courseId": "CS 5002",
    "courseName": "Discrete and Data Structures",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471873",
    "professor": "Gust Philip",
    "courseId": "CS 5003", 
    "courseName": "Recitation for CS 5001",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471874",
    "professor": "Gust Philip",
    "courseId": "CS 5004",
    "courseName": "Object-Oriented Design",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471875",
    "professor": "Gust Philip",
    "courseId": "CS 5005",
    "courseName": "Recitation for CS 5004",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471876",
    "professor": "Gust Philip",
    "courseId": "CS 5006",
    "courseName": "Algorithms",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471877",
    "professor": "Gust Philip",
    "courseId": "CS 5007",
    "courseName": "Computer Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471878",
    "professor": "Gust Philip",
    "courseId": "CS 5600",
    "courseName": "Computer Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471879",
    "professor": "Gust Philip",
    "courseId": "CS 5800",
    "courseName": "Algorithms",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471880",
    "professor": "Ed Katz",
    "courseId": "CS 5001",
    "courseName": "Intensive Foundations of Computer Science",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471881",
    "professor": "Ed Katz",
    "courseId": "CS 5002", 
    "courseName": "Discrete and Data Structures",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471882",
    "professor": "Ed Katz",
    "courseId": "CS 5003", 
    "courseName": "Recitation for CS 5001",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471883",
    "professor": "Ed Katz",
    "courseId": "CS 5004",
    "courseName": "Object-Oriented Design",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471884",
    "professor": "Ed Katz",
    "courseId": "CS 5005",
    "courseName": "Recitation for CS 5004",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471885",
    "professor": "Ed Katz",
    "courseId": "CS 5006", 
    "courseName": "Algorithms",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471886",
    "professor": "Ed Katz",
    "courseId": "CS 5007",
    "courseName": "Computer Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471887",
    "professor": "Ed Katz",
    "courseId": "CS 5600",
    "courseName": "Computer Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471888",
    "professor": "Ed Katz",
    "courseId": "CS 5800",
    "courseName": "Algorithms",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471889",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 6200",
    "courseName": "Information Retrieval",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471890",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 7610",
    "courseName": "Foundations of Distributed Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471891",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 5150",
    "courseName": "Game Artificial Intelligence",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471892",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 5200",
    "courseName": "Database Management Systems",
    "username": "mengbanana",
    "comment": "Too hard"
},
{   
    "_id": "5b21ca3eeb7f6fbccd471893",
    "professor": "Anurag Phardwaj",
    "courseId": "CS 5400", 
    "courseName": "Principles of Programming Language",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471894",
    "professor": "Prasad Saripalli",
    "courseId": "CS 5500",
    "courseName": "Managing Software Development",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471895",
    "professor": "Prasad Saripalli",
    "courseId": "CS 5520",
    "courseName": "Mobile Application Development",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471896",
    "professor": "Chuhua Zhong",
    "courseId": "CS 5610", 
    "courseName": "Web Development",
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471898",
    "professor": "Chuhua Zhong",
    "courseId": "CS 6200",
    "courseName": "Information Retrieval", 
    "username": "mengbanana",
    "comment": "Too hard"
},
{
    "_id": "5b21ca3eeb7f6fbccd471899",
    "professor": "Chuhua Zhong",
    "courseId": "CS 7610", 
    "courseName": "Foundations of Distributed Systems",
    "username": "mengbanana",
    "comment": "Too hard"
}
]


export function getComments() {
  return comments;
}

export function getComment(id) {
  return comments.find(m => m._id === id);
}

/*export function saveComment(comment) {
  let commentInDb = comments.find(m => m._id === comment._id) || {};
  commentInDb.name = comment.name;
  commentInDb.genre = genresAPI.genres.find(g => g._id === comment.genreId);
  commentInDb.numberInStock = comment.numberInStock;
  commentInDb.dailyRentalRate = comment.dailyRentalRate;

  if (!commentInDb._id) {
    commentInDb._id = Date.now();
    comments.push(commentInDb);
  }
  return commentInDb;
}*/

export function deleteComment(id) {
  let commentInDb = comments.find(m => m._id === id);
  comments.splice(comments.indexOf(commentInDb), 1);
  return commentInDb;
}